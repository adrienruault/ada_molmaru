...
Download the `Bahamas Leaks`, `Offshore Leaks`, `Panama papers` and `Paradise papers` zip files. Then unzip them and place the csv files in their corresponding folder following the structure specified below. All the files and folders marked by `(*)` are generated by `preprocessing.ipynb` or `main_pipeline.ipynb`.
