...
from bokeh.models import (
  GMapPlot, GMapOptions, ColumnDataSource, Circle, Range1d, PanTool, WheelZoomTool, BoxSelectTool, HoverTool, Line)
...
class ItemNetwork:
    ...
    def getMap_bokeh(self):
        

        map_options = GMapOptions(map_type="roadmap", zoom=1, scale_control = False)
        fig = GMapPlot(plot_width=666, plot_height=400, x_range=Range1d(), y_range=Range1d(), map_options=map_options)
        fig.title.text = 'Connections from ' + self.central_item.name
        
        fig.api_key = "AIzaSyBWiP1vrOwawG0gqvWPlMz8CAbom7adjJ4"
...
#                        )
            ...
            if (item_pair[0].country != -1 and item_pair[1].country != -1):
                ...
                source = ColumnDataSource(dict(x=[item_pair[0].longitude, item_pair[1].longitude],
                                               y=[item_pair[0].latitude, item_pair[1].latitude],
                                               Type = [relationship_elem.relationship,relationship_elem.relationship],
...
#                    )
        ...
        source = ColumnDataSource(
            data=dict(
                ...
                lon=[self.central_item.longitude],
                Type = [self.central_item.type],
                Name = [self.central_item.name],
                Country = [self.central_item.country],
            )
        )    

        circle = Circle(x="lon", y="lat", size=12, fill_color=color_dic[self.central_item.type], fill_alpha=0.6, 
